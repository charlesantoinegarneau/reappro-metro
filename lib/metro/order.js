...
export function validOrder(order,week){
 ...
 const lines=order.lines.map(l=>{
  ...
  return {metro:l.metro,sku:l.sku,product:l.product||'',qty:l.qty,base:Number.isInteger(l.base)?l.base:null,jevQty:l.jevQty??null,confidence:l.confidence??null};
 });
 return {week,settings,lines};
}
const cell=v=>{const s=String(v??'');return /[",;\n]/.test(s)?'"'+s.replace(/"/g,'""')+'"':s;};
// One file per Metro, only the lines to order.
