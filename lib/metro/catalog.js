...
//   variant barcode: exact brand (Vendor), product (Title) and variant.
...
export function parseShopifyExport(text){
 ...
 const col={handle:at('Handle'),title:at('Title'),vendor:at('Vendor'),barcode:at('Variant Barcode'),options:['Option1 Value','Option2 Value','Option3 Value'].map(at)};
 ...
 for(const record of records){
  ...
  out[code]={brand:first.brand||'Autre',product:first.product,variant};
 }
...
export function validCatalog(catalog){
 ...
 for(const [code,v] of entries){
  if(!/^\d{1,20}$/.test(code)||typeof v?.brand!=='string'||typeof v.product!=='string'||typeof (v.variant??'')!=='string'||v.brand.length>120||v.product.length>250||(v.variant??'').length>200)throw Error('Catalogue invalide.');
  out[code]={brand:v.brand,product:v.product,variant:v.variant||''};
 }
 return out;
}
// The catalog wins when it knows the barcode; the description otherwise.
export const identify=(sku,description,catalog)=>catalog?.[barcodeKey(sku)]||describe(description);
