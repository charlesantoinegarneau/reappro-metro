...
import {addVariant,barcodeKey} from './catalog.js';
...
export const CATALOG_QUERY=`query MetroCatalog($cursor:String){productVariants(first:250,after:$cursor){nodes{
 barcode sku selectedOptions{name value} product{title vendor} inventoryItem{unitCost{amount currencyCode}}
...
// One page → {items: {barcode: {brand, product, variant, cost, sku, also?}}, next}.
...
export async function catalogPage(gql,cursor=null){
 ...
 for(const v of page.nodes){
  ...
  addVariant(items,code,{brand:v.product.vendor||'Autre',product:v.product.title,variant,cost:Number.isFinite(amount)&&amount>=0?amount:null,sku:v.sku||''});
 }
