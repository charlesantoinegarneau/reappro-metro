...
import {articleVariants,barcodeKey,nameMatcher} from './catalog.js';
...
// Rows + catalog → {items: {barcode: stock}, matched, unmatched}. Two
// variants of the same article (« Gratuit! », « Dernière Chance! ») are one
// shelf: their stocks add up (received as one, sold as the other).
export function matchStock(rows,catalog){
 const bySku=new Map(),byName=nameMatcher(catalog);
 for(const [code,entry] of Object.entries(catalog||{}))for(const v of articleVariants(entry))if(v.sku)bySku.set(norm(v.sku),code);
 const items={},unmatched=[];
 for(const r of rows){
  const code=(r.sku&&bySku.get(norm(r.sku)))||byName(r.product),k=code&&barcodeKey(code);
  if(k)items[k]=(items[k]??0)+r.stock;else unmatched.push(r.product||r.sku);
 }
