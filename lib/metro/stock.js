...
export function matchStock(rows,catalog){
 ...
 for(const [code,v] of Object.entries(catalog||{})){
  if(v.sku)bySku.set(norm(v.sku),code);
  byName.set(norm(v.variant?`${v.product} — ${v.variant}`:v.product),code);
  if(!v.variant)byName.set(norm(`${v.product} — Default Title`),code);
 }
 const items={},unmatched=[];
 for(const r of rows){
  const code=(r.sku&&bySku.get(norm(r.sku)))||byName.get(norm(r.product));
