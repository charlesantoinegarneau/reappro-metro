...
export function parseSalesTable(table){
 ...
 const byKey=new Map(),dates=new Map();let skipped=0,from='',to='';
 for(const r of grid.slice(at+1)){
  ...
  if(!from||date<from)from=date;if(date>to)to=date;
 ...
 return {rows:validateSalesRows(rows),skipped,hasStock:index.stock!==undefined,from,to};
}
