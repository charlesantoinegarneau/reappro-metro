...
// `invoices` ([{metro, date, items: {barcode: qty}}]) what was delivered since.
export function planLines(rows,settings,today,catalog=null,stocks=null,invoices=[]){
 ...
 // Every week, the one in progress included, for the sales since a stock count.
 const allWeeks=new Map();for(const r of rows){const k=r.metro+'|'+r.sku;if(!allWeeks.has(k))allWeeks.set(k,[]);allWeeks.get(k).push(r);}
 const lines=[];
 for(const [key,{metro,sku,list,history,scaled}] of series){
  ...
  const stockIssue=shelf!==null&&shelf<0?'négatif':null;
  // Since the count: + deliveries (invoices after its date), − sales after it.
  const since=fileStock!==undefined&&!stockIssue?sinceCount(stocks[metro].asOf,barcodeKey(sku),invoices.filter(i=>i.metro===metro),allWeeks.get(key)||[]):null;
  const stock=stockIssue?null:since?Math.max(0,Math.round(shelf+since.delivered-since.sold)):shelf;
  ...
  if(since)rule+=` · stock estimé ${stock} = compté ${shelf}${since.delivered?` + livré ${since.delivered}`:''}${since.sold?` − vendu ${round(since.sold)}`:''}`;
  ...
  lines.push({key,metro,sku,product,brand,name,variant,cost,week,pack,stock,stockIssue,stockRaw:shelf,stockAsOf:fileStock!==undefined?stocks[metro].asOf||null:null,stockCounted:since?shelf:null,delivered:since?.delivered||0,soldSince:since?round(since.sold):0,history,v4:round(v4),v12:round(v12),trend,urgency,base,rule,ruptures,boNow,candidates:options,coverage});
 }
...
// Deliveries and sales after a stock count (end of its day). An invoice is
// delivered the day it is created; one of the count's day is already in the
// count (the transfer is in Shopify). Sales come by week: the week of the
// count keeps the share of its reported days after that day (the days of a
// report run from Thursday).
function sinceCount(asOf,code,invoices,weeks){
 if(!asOf)return null;
 const delivered=invoices.reduce((n,i)=>n+(i.date>asOf?i.items[code]||0:0),0);
 let sold=0;
 for(const r of weeks){
  if(addDays(r.week,(r.days||7)-1)<=asOf)continue;
  const counted=r.week>asOf?0:(Date.parse(asOf)-Date.parse(r.week))/864e5+1;
  sold+=Math.max(0,r.units)*((r.days||7)-counted)/(r.days||7);
 }
 return delivered||sold>0?{delivered,sold}:null;
}
