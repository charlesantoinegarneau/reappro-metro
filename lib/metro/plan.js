...
export function planLines(rows,settings,today,catalog=null){
 ...
 for(const [key,{metro,sku,list,history,scaled}] of series){
  ...
  const {brand,product:name,variant}=identify(sku,product,catalog),ruptures=history.filter(h=>h.rupture).length,boNow=history.at(-1)?.rupture==='reseau';
  ...
  lines.push({key,metro,sku,product,brand,name,variant,week,pack,stock,history,v4:round(v4),v12:round(v12),trend,urgency,base,rule,ruptures,boNow,candidates:candidates(base,pack,v12>0)});
 }
