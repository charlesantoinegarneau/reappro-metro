...
export default function MetroPanel(){
 ...
 // Shelf stock of the Metro selected: the replenishment export of the Shopify tool.
 async function uploadStock(file){
  if(!file)return;setBusy('stock');setError('');setNotice('');
  try{const rows=await readStockFile(file.name,await file.arrayBuffer()),asOf=stockDate(file.name);
   const j=await api('',{action:'stock',metro:scope,rows,filename:file.name,asOf});
   setNotice(`Stock en rayon de ${scope}${asOf?` au ${dateLabel(asOf)}`:''} : ${j.matched} produits reconnus${j.negative?`, dont ${j.negative} en stock négatif (erreur d’inventaire, ignoré)`:''}${j.unmatched?` · ${j.unmatched} ligne${j.unmatched>1?'s':''} du fichier sans correspondance (ex. ${j.unmatchedSample.join(' ; ')})`:''}. Relancez Jev pour qu’il en tienne compte.`);await load();
  }catch(e){setError(e.message);}finally{setBusy('');}
 }
 // Invoices « Commande interne » (PDF): deliveries to each Metro, added to its
 // stock after the date of its stock file. The Metro is read on the invoice.
 async function uploadInvoices(files){
  if(!files.length)return;setBusy('invoice');setError('');setNotice('');
  const done=[];
  try{for(const file of files){
    let invoice;try{invoice=await readInvoicePdf(await file.arrayBuffer());}catch(e){throw Error(file.name+' : '+e.message);}
    if(!invoice.metro)throw Error(`${file.name} : client « ${invoice.client} » inconnu (Innovation, St-Augustin, St-Nicolas ou Ste-Foy attendu).`);
    const j=await api('',{action:'invoice',invoice});
    done.push(`n° ${j.number} (${j.metro}, ${dateLabel(j.date)}) : ${j.units} unités, ${j.matched} ligne${j.matched>1?'s':''} sur ${j.lines} reconnue${j.matched>1?'s':''}${j.unmatched?` (sans correspondance : ${j.unmatchedSample.join(' ; ')}${j.unmatched>5?'…':''})`:''}${j.counted?' · ajoutée au stock':j.asOf?` · déjà comprise dans le stock au ${dateLabel(j.asOf)}`:' · importez le stock de ce Metro pour qu’elle compte'}`);
   }
   setNotice(`Facture${done.length>1?'s':''} ${done.join(' · ')}. Relancez Jev pour qu’il en tienne compte.`);
  }catch(e){setError((done.length?`${done.length} facture${done.length>1?'s':''} importée${done.length>1?'s':''}. `:'')+e.message);}finally{setBusy('');await load();}
 }
 async function deleteInvoice(number){
  if(!confirm(`Retirer la facture n° ${number} ? Ses livraisons ne compteront plus dans le stock.`))return;
  setBusy('invoice');setError('');
  try{await api('',{action:'invoice-delete',number});setNotice(`Facture n° ${number} retirée.`);await load();}catch(e){setError(e.message);}finally{setBusy('');}
 }
 ...
 const stockPicker=<label className={'quiet-button monthly-upload'+(busy==='stock'?' busy':'')}>{busy==='stock'?'Lecture…':`Importer le stock en rayon (${scope})`}<input type="file" accept=".xlsx,.csv" hidden disabled={!!busy||!scope} onChange={e=>{uploadStock(e.target.files?.[0]);e.target.value='';}}/></label>;
 const invoicePicker=<label className={'quiet-button monthly-upload'+(busy==='invoice'?' busy':'')}>{busy==='invoice'?'Lecture…':'Déposer des factures (PDF)'}<input type="file" accept=".pdf,application/pdf" multiple hidden disabled={!!busy} onChange={e=>{uploadInvoices([...(e.target.files||[])]);e.target.value='';}}/></label>;
 ...
 // « Confiance Jev plus grand que x % » : lines Jev has not decided have no confidence and are hidden.
 const confAbove=minConf===''?null:Number(minConf)/100;
 const filtered=!!query.trim()||above!==null||confAbove!==null||onlyReview;
 const visible=l=>(!onlyReview||review(l))&&(!query.trim()||matches(l,query))&&(above===null||qtyOf(l)>above)&&(confAbove===null||decisions[l.key]?.source==='jev'&&decisions[l.key].confidence>confAbove);
 ...
 return <section className="comparison metro-panel" id="metro">
  <div className="sectionhead"><div><h1>Commandes Metro{data&&<> — semaine du {dateLabel(data.week)}</>}</h1>{mode==='prepare'&&data?.weeks?.count>0&&<p className="footnote">Ventes du {dateLabel(data.weeks.first)} au {dateLabel(data.weeks.last)} ({data.weeks.count} semaine{data.weeks.count>1?'s':''} complète{data.weeks.count>1?'s':''}){data.weeks.current?.map(c=><Fragment key={c.week}> · semaine du {dateLabel(c.week)} en cours ({c.metros.map(m=>`${m.metro} : ${m.days} jour${m.days>1?'s':''}`).join(', ')}), pas encore dans le calcul</Fragment>)}{data.files?.[0]&&<> · dernier fichier : {data.files[0].name}</>}. Chaque fichier remplace les semaines qu’il couvre. {data.catalog?<>Catalogue Shopify ({data.catalog.source==='shopify'?'lu dans Shopify':'export CSV'}) : {data.catalog.count} variantes{data.catalog.priced?`, ${data.catalog.priced} avec coût`:''}, mis à jour le {new Date(data.catalog.importedAt).toLocaleDateString('fr-CA')}. {lines.filter(l=>l.cost!=null).length} lignes sur {lines.length} ont un coût.</>:<>Marques déduites des descriptions Metro; le catalogue Shopify donne les noms exacts et le coût de chaque produit.</>}{data.stocks&&Object.keys(data.stocks).length>0&&<> Stock en rayon : {Object.entries(data.stocks).map(([m,v])=>`${m}${v.asOf?` au ${dateLabel(v.asOf)}`:''} (${v.matched} produits${v.negative?`, ${v.negative} négatif${v.negative>1?'s':''} ignoré${v.negative>1?'s':''}`:''})`).join(' · ')}.</>}{data.invoices?.some(i=>i.counted)&&<> Livraisons ajoutées depuis : {Object.entries(data.invoices.filter(i=>i.counted).reduce((o,i)=>({...o,[i.metro]:[...(o[i.metro]||[]),i]}),{})).map(([m,list])=>`${m} (${list.map(i=>`n° ${i.number} du ${dateLabel(i.date)}`).join(', ')})`).join(' · ')}.</>}</p>}</div>{mode==='prepare'&&lines.length>0&&<span className="metro-imports">{picker}{stockPicker}{invoicePicker}{shopifyButton||catalogPicker}</span>}</div>
  ...
  {lines.length>0&&<>
   ...
   {mode==='pick'&&<div className="pick">
    ...
    {pickGroups.map(g=><section key={g.brand} className="pick-group"><h3>{g.brand}</h3>{g.lines.map(l=>{const st=status[l.key],q=qtyOf(l),was=plannedOf(l);return <article key={l.key} className={'pick-line'+(st?' '+st:'')}>
     ...
     <div className="pick-info"><b>{l.name}</b>{l.variant&&<span>{l.variant}</span>}<small className="metro-sku">{l.sku}</small>{l.stock!==null&&<small>en rayon : {l.stock}</small>}{q!==was&&st!=='removed'&&<small className="pick-changed">prévu {was}</small>}{l.boNow&&<small className="metro-bo">BO probable</small>}</div>
   ...
   <div className="table-scroll"><table className="metro-table"><thead><tr><th scope="col">Produit</th><th scope="col">Semaines<small>vert : 4 dernières</small></th><th scope="col">Rythme<small>/ sem. (4 dern. · toutes)</small></th><th scope="col">Stock</th><th scope="col">Règle</th><th scope="col">Jev<small>choix · confiance</small></th><th scope="col">À commander</th></tr></thead>
    <tbody>{groups.map(g=><Fragment key={g.brand}><tr className="metro-brand"><th scope="rowgroup" colSpan={7}>{g.brand}<small>{g.lines.length} produit{g.lines.length>1?'s':''} · {g.lines.reduce((n,l)=>n+keptOf(l),0)} unités à commander{g.lines.some(l=>l.cost!=null)&&<> · {money(g.lines.reduce((n,l)=>n+(l.cost??0)*keptOf(l),0))}</>}</small></th></tr>{g.lines.map((l,i)=>{const d=decisions[l.key],q=qtyOf(l),same=i>0&&g.lines[i-1].name===l.name&&l.variant;return <tr key={l.key} className={(review(l)?'metro-review':'')+(status[l.key]==='removed'?' metro-removed':'')}>
     ...
     <td title={l.stockCounted!=null?`Compté ${l.stockCounted} au ${dateLabel(l.stockAsOf)}${l.delivered?` + livré ${l.delivered}`:''}${l.soldSince?` − vendu ${l.soldSince}`:''} = ${l.stock} (estimé)`:l.stockAsOf?`Stock en rayon au ${dateLabel(l.stockAsOf)}`:''}>{l.stockIssue?<small className="negative">{l.stockRaw} · à corriger</small>:l.stock===null?'—':l.stock}{l.stockCounted!=null&&<small>estimé{l.delivered?` · +${l.delivered} livré`:''}</small>}{l.urgency!=='normale'&&<small className="negative">{l.urgency==='rupture'?'rupture proche':'urgent'}</small>}</td>
     <td title={l.rule}>{l.base}{l.coverage?.regle!==undefined&&<small className="metro-cover">couvre {pct(l.coverage.regle)}</small>}</td>
     <td title={d?.probabilities?Object.entries(d.probabilities).map(([k,p])=>`${LABEL[k]} (${l.candidates[k]}) : ${pct(p)}${l.coverage?.[k]!==undefined?` · couvre ${pct(l.coverage[k])}`:''}`).join('\n'):d?.note||''}>{d?.source==='jev'?<><b>{d.qty}</b><span className="metro-confidence"><i style={{width:pct(d.confidence??0)}}/></span><small>{d.label?LABEL[d.label]+' · ':''}{pct(d.confidence??0)}</small>{d.label&&l.coverage?.[d.label]!==undefined&&<small className="metro-cover">couvre {pct(l.coverage[d.label])}</small>}</>:d?<small>règle{d.note?' (Jev indisponible)':''}</small>:'—'}</td>
   ...
   {data.invoices?.length>0&&<details className="monthly-older"><summary>Factures déposées ({data.invoices.length})</summary><ul className="metro-invoices">{data.invoices.map(i=><li key={i.number}><span><b>n° {i.number}</b> · {i.metro} · {dateLabel(i.date)} · {i.items} unités · {money(i.total)} · {i.matched}/{i.lines} lignes reconnues{i.unmatched?` (${i.unmatched} sans correspondance)`:''} · <em>{i.counted?'ajoutée au stock':data.stocks?.[i.metro]?.asOf?`comprise dans le stock au ${dateLabel(data.stocks[i.metro].asOf)}`:'aucun stock importé pour ce Metro'}</em></span><button className="quiet-button" onClick={()=>deleteInvoice(i.number)} disabled={!!busy}>Retirer</button></li>)}</ul><p className="footnote">Une facture est livrée le jour où elle est créée. Elle s’ajoute au stock en rayon seulement si elle est postérieure au fichier de stock du Metro (le transfert est déjà compté dans Shopify). Le stock estimé = stock compté + livré depuis − vendu depuis (rapports Metro).</p></details>}
   </>}
  </>}
 </section>;
}
