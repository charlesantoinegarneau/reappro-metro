...
export default function MetroPanel(){
 ...
 const [scope,setScope]=useState(''),[onlyReview,setOnlyReview]=useState(false),[query,setQuery]=useState(''),[added,setAdded]=useState([]),[busy,setBusy]=useState(''),[progress,setProgress]=useState(null),[error,setError]=useState(''),[notice,setNotice]=useState('');
 ...
 async function uploadCatalog(file){
  ...
  try{const items=await readCatalogFile(await file.arrayBuffer()),j=await api('',{action:'catalog',items});
   setNotice(`Catalogue Shopify importé : ${j.count} variantes avec code-barres. Marques, produits et variantes viennent maintenant de Shopify quand le code-barres correspond.`);await load();
  }catch(e){setError(e.message);}finally{setBusy('');}
 }
 ...
 const shown=lines.filter(l=>l.metro===scope&&(!onlyReview||review(l))&&(!query.trim()||matches(l,query)));
 ...
 const totals=metros.map(m=>{const list=lines.filter(l=>l.metro===m);return {metro:m,lines:list.filter(l=>qtyOf(l)>0).length,units:list.reduce((n,l)=>n+qtyOf(l),0),review:list.filter(l=>decisions[l.key]?.review).length,bo:list.filter(l=>l.boNow).length,decided:list.filter(l=>decisions[l.key]).length,total:list.length};});
 const setting=(k,v)=>setSettings(s=>({...s,[k]:v}));
 return <section className="comparison metro-panel" id="metro">
  <div className="sectionhead"><div><h1>Commandes Metro{data&&<> — semaine du {dateLabel(data.week)}</>}</h1>{data?.weeks?.count>0&&<p className="footnote">Ventes du {dateLabel(data.weeks.first)} au {dateLabel(data.weeks.last)} ({data.weeks.count} semaines){data.files?.[0]&&<> · dernier fichier : {data.files[0].name}</>}. Chaque fichier remplace les semaines qu’il couvre. {data.catalog?<>Catalogue Shopify : {data.catalog.count} variantes, importé le {new Date(data.catalog.importedAt).toLocaleDateString('fr-CA')}.</>:<>Marques déduites des descriptions Metro; le catalogue Shopify donne les noms exacts.</>}</p>}</div>{lines.length>0&&<span className="metro-imports">{picker}{catalogPicker}</span>}</div>
  ...
  {lines.length>0&&<>
   ...
   <div className="metro-cards">{totals.map(t=><button key={t.metro} aria-pressed={scope===t.metro} onClick={()=>setScope(t.metro)}><b>{t.metro}</b><span>{t.lines} produit{t.lines>1?'s':''} à commander · {t.units} unités</span><small>{t.decided?`Jev : ${t.decided}/${t.total}${t.review?` · ${t.review} à vérifier`:''}`:'Règle seulement'}{t.bo?` · ${t.bo} en BO probable`:''}</small></button>)}</div>
   <div className="metro-search"><input type="search" placeholder={`Chercher un produit, une marque ou un code-barres (${scope})`} aria-label="Chercher un produit" value={query} onChange={e=>setQuery(e.target.value)}/>{query&&<span>{shown.length} résultat{shown.length>1?'s':''}</span>}</div>
   ...
   <div className="table-scroll"><table className="metro-table"><thead><tr><th scope="col">Produit</th><th scope="col">Semaines<small>vert : 4 dernières</small></th><th scope="col">Rythme<small>/ sem. (4 dern. · toutes)</small></th><th scope="col">Stock</th><th scope="col">Règle</th><th scope="col">Jev<small>choix · confiance</small></th><th scope="col">À commander</th></tr></thead>
    <tbody>{groups.map(g=><Fragment key={g.brand}><tr className="metro-brand"><th scope="rowgroup" colSpan={7}>{g.brand}<small>{g.lines.length} produit{g.lines.length>1?'s':''} · {g.lines.reduce((n,l)=>n+qtyOf(l),0)} unités à commander</small></th></tr>{g.lines.map((l,i)=>{const d=decisions[l.key],q=qtyOf(l),same=i>0&&g.lines[i-1].name===l.name&&l.variant;return <tr key={l.key} className={review(l)?'metro-review':''}>
   ...
   {!shown.length&&<p className="footnote">{query?`Aucun produit vendu à ${scope} ne correspond à « ${query} ».`:`Aucune ligne à vérifier pour ${scope}.`}</p>}
