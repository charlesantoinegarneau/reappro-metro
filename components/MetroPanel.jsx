...
export default function MetroPanel(){
 ...
 return <section className="comparison metro-panel" id="metro">
  ...
  {lines.length>0&&<>
   ...
   <div className="metro-cards">{totals.map(t=><button key={t.metro} aria-pressed={scope===t.metro} onClick={()=>setScope(t.metro)}><b>{t.metro}</b><span>{t.lines} produit{t.lines>1?'s':''} à commander · {t.units} unités{t.value>0&&<> · {t.priced?'':'≥ '}{money(t.value)}</>}</span><small>{t.decided?`Jev : ${t.decided}/${t.total}${t.review?` · ${t.review} à vérifier`:''}`:'Règle seulement'}{t.bo?` · ${t.bo} en BO probable`:''}</small></button>)}</div>
