...
export async function GET(req){
 ...
 try{const db=database(),today=localDate(new Date()),week=planWeek(today),data=await sales(db,user);
  const saved=await db.prepare('SELECT document,saved_at FROM metro_orders WHERE user_id=? AND week=?').bind(user,week).first();
  const weeks=[...new Set(data.rows.map(r=>r.week))].sort();
  return json({week,settings,lines:planLines(data.rows,settings,today),files:data.files,importedAt:data.importedAt,weeks:{first:weeks[0]||null,last:weeks.at(-1)||null,count:weeks.length},order:saved?{...JSON.parse(saved.document),savedAt:saved.saved_at}:null,jev:!!jev()});
...
export async function POST(req){
 ...
 try{
  ...
  if(body.action==='decide'){
   ...
   const wanted=new Set(body.keys),lines=planLines((await sales(db,user)).rows,settings,today).filter(l=>wanted.has(l.key));
