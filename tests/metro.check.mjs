...
import {detectColumns,mergeSales,parseDate,parseNumber,parseSalesTable,readSalesFile,validateSalesRows,weekOf} from '../lib/metro/sales.js';
...
// Fictitious Metro sales: 12 weeks ending the week of Sep 14 2026.
const WEEKS=Array.from({length:12},(_,i)=>{const d=new Date(Date.UTC(2026,5,29+i*7));return d.toISOString().slice(0,10);});
const row=(metro,sku,week,units,extra={})=>({metro,sku,product:'Produit '+sku,week,units,stock:null,pack:null,...extra});

test('lecture : CSV à point-virgule, titre, dates québécoises, nombres à virgule, semaines',async()=>{
 assert.equal(weekOf('2026-09-25'),'2026-09-21');assert.equal(weekOf('2026-09-21'),'2026-09-21');assert.equal(weekOf('2026-09-27'),'2026-09-21');
 assert.equal(parseDate('14/09/2026'),'2026-09-14');assert.equal(parseDate('2026-09-14 00:00:00'),'2026-09-14');assert.equal(parseDate('46279'),'2026-09-14');assert.equal(parseDate('2026-02-30'),null);
 ...
 assert.deepEqual(rows.find(r=>r.sku==='ABC-1'),{metro:'Metro Charlesbourg',sku:'ABC-1',product:'Protéine vanille',week:'2026-09-14',units:3.5,stock:3,pack:6});
...
test('fusion : un fichier remplace les semaines qu’il couvre, 26 semaines conservées',()=>{
 const old=[row('A','X','2026-09-07',5),row('A','X','2026-09-14',9),row('B','X','2026-09-14',2)];
 const merged=mergeSales(old,[row('A','Y','2026-09-14',1)]);
 assert.deepEqual(merged.map(r=>[r.metro,r.sku,r.week]),[['A','X','2026-09-07'],['A','Y','2026-09-14'],['B','X','2026-09-14']]);
 assert.equal(mergeSales([row('A','X','2026-01-05',1)],[row('A','X','2026-09-14',1)]).length,1);
 assert.throws(()=>validateSalesRows([row('A','X','2026-09-15',1)]),/invalide/,'une semaine commence un lundi');
 assert.throws(()=>validateSalesRows([row('A','X','2026-09-14',1),row('A','X','2026-09-14',2)]),/double/)});

test('règle : rythme, tendance, stock, caisse, semaine en cours exclue, semaine absente ≠ zéro',()=>{
 assert.equal(planWeek(TODAY),'2026-09-28');assert.equal(planWeek('2026-09-28'),'2026-10-05');
 ...
 const rows=[
  ...
  // Rising: 2 / week then 10 / week over the last 4 weeks, no stock: 10/7 × 7 × 1.15 = 11.5 → 12.
  ...WEEKS.map((w,i)=>row('A','RISING',w,i>=8?10:2)),
  // Sold once long ago: nothing recent.
  row('A','OLD',WEEKS[0],0),
  // The current week (not finished) is ignored.
  row('A','STEADY','2026-09-21',100,{stock:4,pack:6}),
 ...
 assert.equal(steady.week,'2026-09-28');assert.equal(steady.history.length,12);assert.equal(steady.v4,7);assert.equal(steady.trend,'stable');assert.equal(steady.stock,4);assert.equal(steady.base,12);
 ...
 const rising=get('A','RISING');assert.equal(rising.trend,'hausse');assert.equal(rising.stock,null);assert.equal(rising.base,12);assert.match(rising.rule,/stock inconnu/);
...
test('Jev : une question « choice » par ligne, probabilités, confiance faible à vérifier, repli sur la règle',async()=>{
 ...
 assert.deepEqual(down.filter(d=>d.source==='regle'&&d.review).map(d=>[d.qty,d.note]),[[2,'panne'],[12,'panne']]);
...
test('commande : validation, CSV par Metro, enregistrement par semaine',async()=>{
 const order={week:'2026-09-28',settings:S,lines:[{metro:'A',sku:'X',product:'Protéine, vanille',qty:12,base:12,jevQty:18,confidence:.8},{metro:'A',sku:'Y',product:'',qty:0},{metro:'B',sku:'X',product:'P',qty:6}]};
 const valid=validOrder(order,'2026-09-28');assert.equal(valid.lines[1].jevQty,null);
 assert.throws(()=>validOrder(order,'2026-10-05'),/semaine/);assert.throws(()=>validOrder({...order,lines:[{metro:'A',sku:'X',qty:1.5}]},'2026-09-28'));
 ...
 assert.equal(JSON.parse((await t.db.prepare('SELECT document FROM metro_orders WHERE user_id=? AND week=?').bind('owner','2026-09-28').first()).document).lines.length,1);
